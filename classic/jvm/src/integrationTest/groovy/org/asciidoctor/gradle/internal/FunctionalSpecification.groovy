...
class FunctionalSpecification extends Specification implements FunctionalTestFixture {
    ...
    File getJvmConvertGroovyBuildFile(String extraContent, String plugin = 'org.asciidoctor.jvm.convert.classic') {
        buildFile << """
            plugins {
                id '${plugin}'
            }
    ...
    File getJvmConvertKotlinBuildFile(String extraContent, String plugin = 'org.asciidoctor.jvm.convert.classic') {
        buildFileKts << """
            plugins {
                id ("${plugin}")
            }

            ${getOfflineRepositories(DslType.KOTLIN_DSL)}
