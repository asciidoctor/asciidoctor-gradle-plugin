...
/**
 * Implementation of a PDF theme residing on Github.
 *
 ...
 */
@CompileStatic
class DefaultGithubThemeCollection implements GithubThemeCollection {
    final String name
    final Provider<Directory> themeDir
    private final GitHubArchive archive
    ...
    @Inject
    DefaultGithubThemeCollection(String name, Project tempProjectReference) {
        this.name = name
        this.stringTools = ConfigCacheSafeOperations.from(tempProjectReference).stringTools()
        this.archive = new GitHubArchive(tempProjectReference)
    ...
    static class Factory implements NamedDomainObjectFactory<GithubThemeCollection> {
         ...
         */
        @Override
        GithubThemeCollection create(String name) {
            objectFactory.newInstance(DefaultGithubThemeCollection, name)
        }
    }
}
