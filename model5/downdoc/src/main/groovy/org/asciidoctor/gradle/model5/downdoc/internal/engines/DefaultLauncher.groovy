...
//        final warnings = conversionSettings.fatalWarnings.getOrElse(Collections.EMPTY_SET)
        ...
        groups.each { parent, files ->
            ...
            files.each { partition ->
                final destFileName = partition.name.replaceFirst(~/(\.a\p{Alpha}+?)$/, '.md')
                final destArgs = ['exec', 'downdoc', '-o', new File(destPath, destFileName).absolutePath]
                final result = execTools.exec(CAPTURE, CAPTURE) { spec ->
                    es.copyTo(spec)
                    spec.tap {
                        args(destArgs)
                        args(attrs)
                        args(partition.absolutePath)
...
//        parseLogs(stringTools, jobLogDir.get(), warnings, index)
    ...
    @SuppressWarnings(['DuplicateNumberLiteral', 'UnusedPrivateMethodParameter'])
