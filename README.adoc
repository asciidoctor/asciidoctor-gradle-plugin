...
sourceDocumentNames:: an override to process multiple source files, which may be a subset of all
  files available in [x-]`${sourceDir}`.
  Use `sourceDocumentNames` to append and `setSourceNames` to overwrite.
  Type: FileCollection, but any collection of objects convertible with `project.files` can be passed
  Default: All files in [x-]`${sourceDir}`.
...
sourceDir:: the default value has changed from `src/asciidoc` to `src/docs/asciidoc`.
...
For backwards compatibility with older version, embedding `attributes` within `options` are still allowed, including
legacy forms.
