...
The base directory is now set to the directory of the root project. This may influence how your includes are processed. Either set `baseDir` to specific directory or set `includedir` to `sourceDir` (This might be changed before final release depending on feedback).
