...
ifdef::env-github[]
NOTE: This is the maintenance branch for the 1.6.x series. `1.6.0` is basically `1.5.10` repackaged. Only small maintenance releases are performed on this branch. For all new development on the 2.x series please look at the `development` branch.
endif::[]
...
NOTE: The 1.6.x versions of this plugin is NOT comatible with https://github.com/spring-projects/spring-restdocs[Spring REST Docs]. Use v1.5.11 or v1.5.12 of this plugin instead and do not manually set the version of AsciidoctorJ in your build script.
...
NOTE: Custom extensions that works against 1.5.x might need recompilation against AsciidoctorJ 1.6.x in order to work with the 1.6.x series of these plugins.
