= Asciidoctor Gradle Plugin
Andres Almiray <https://github.com/aalmiray[@aalmiray]>
:asciidoc-url: https://asciidoc.org
:asciidoctor-url: https://asciidoctor.org
:gradle-url: http://gradle.org/
:asciidoctor-development-docs: https://asciidoctor.github.io/asciidoctor-gradle-plugin/
:project-full-path: asciidoctor/asciidoctor-gradle-plugin
:github-branch: development-5.x
:linkattrs:
ifndef::env-github[:icons: font]
ifdef::env-github,env-browser[]
:toc: preamble
:toclevels: 2
endif::[]
ifdef::env-github[]
:status:
:outfilesuffix: .adoc
:!toc-title:
:note-caption: :paperclip:
:important-caption: :exclamation:
:warning-caption: :warning:
endif::[]

ifdef::status[]
image:https://github.com/asciidoctor/asciidoctor-gradle-plugin/workflows/Build/badge.svg[GitHub Actions Status,link=https://github.com/asciidoctor/asciidoctor-gradle-plugin/actions]
image:http://img.shields.io/coveralls/{project-full-path}/{github-branch}.svg[Coverage Status,link=https://coveralls.io/r/{project-full-path}]
image:http://img.shields.io/badge/license-ASF2-blue.svg[Apache License 2,link=http://www.apache.org/licenses/LICENSE-2.0.txt]
image:https://gitlab.com/asciidoctor/asciidoctor-gradle-plugin/badges/master/pipeline.svg[link="https://gitlab.com/asciidoctor/asciidoctor-gradle-plugin/commits/master",title="pipeline status"]
endif::[]

The {doctitle} is the official means of using {asciidoctor-url}[Asciidoctor] to convert all your {asciidoc-url}[AsciiDoc] documentation using {gradle-url}[Gradle].

[WARNING]
====
With the release of 5.0.0 and after 12 years of involvement, Schalk Cronjé (https://github.com/ysb33r[@ysb33r]) has retired as the leading maintainer.
Although he is still be around for the odd bugfix and to provide advice, *a new maintainer is urgently needed*.
====

Documentation:: We are migrating our documentation to the new Asciidoctor Antora-based site when it is ready.
In the meantime you can read a snapshot of the current documentation at {asciidoctor-development-docs}{github-branch}

ifdef::env-github[]
Structure:: `master` represents the code for the latest 5.x release of these plugins.
* Development for 5.x is against the link:https://github.com/asciidoctor/asciidoctor-gradle-plugin/tree/development-5.x[{github-branch}] branch.
PRs are preferably taking against this branch.
* The link:https://github.com/asciidoctor/asciidoctor-gradle-plugin/tree/development-6.x[development-6.x] branch tracks the `development-5.x` branch is primarily used to track changes for a future release where all of the classic plugins are dropped and the build requires a minimum of JDK17 and Gradle 9.
* The 3.x & 4.x series of the plugin is now in maintenance-only mode.
PRs for that should be raised against the link:https://github.com/asciidoctor/asciidoctor-gradle-plugin/tree/maintenance-3.x[maintenance-3.x] anf link:https://github.com/asciidoctor/asciidoctor-gradle-plugin/tree/maintenance-4.x[maintenance-4.x] branches.
endif::[]
