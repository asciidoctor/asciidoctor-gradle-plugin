...
files available in `${sourceDir}`. Type: FileCollection. Defaults to all files in `${sourceDir}`.
...
----
repositories {
    mavenLocal() // for resolving the snapshot
    jcenter()    // for resolving all other dependencies
}
