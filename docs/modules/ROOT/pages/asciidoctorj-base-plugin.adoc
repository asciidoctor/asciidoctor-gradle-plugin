...
[horizontal]
include::partial$asciidoctorj-entities.adoc[]
...
[horizontal]
include::partial$common-entities.adoc[]
...
link:{asciidoc-userguide}attributes/assignment-precedence[Attribute assignment precedence, window="_blank"]
...
<4> Version of Groovy Extensions DSL. If not specified and no extensions are specified, {groovy-dsl-name} will not be used. However, if any extensions are added without setting an explicit version and default version will be used.
...
When using the Kotlin DSL the same settings can be achieved use something similar `getModules().getPdf().version("1.2.3")`. In a similar fashion shortcuts can be achieved in the {groovy-dsl-name}:
